...
function saveUser (req, res){
	...
	if(params.name && params.surname && params.nick && params.email && params.password){
		...
		//Encripta contraseña y guarda los datos
		

	}else{
		res.status(200).send({
			message: 'Envía todos loscampos necesarios'
		});
	}
...
//Edición de fdatos del ususario
function updateUser(req,res){
	...
	if(userId != req.user.sub){
		return res.status(500).send({message:'No tiene permiso para actualizar datos del usuario'});
	}
